...
在检索能力稳定后，聚焦于用户体验的全面提升。包括更智能的查询处理、可追溯的引用来源、流畅的多轮对话，以及自建的 Web 前端。
...
- [x] 代码关系图谱（函数调用图、模块依赖图可视化）
- [x] 知识库质量评估工具（自动评测检索准确率与回答质量）
